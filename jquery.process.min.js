...
window.setTimeout(function(){process[tuple[0]+"With"](jqXHR,args);},1);return this;};process[tuple[0]+"With"]=function(obj,args){console.log('Process: '+tuple[0]+' on ',obj);if('notify'===tuple[0]&&'string'===typeof args[0]){args[0]=args[0].replace(' ','_');$.fn.trigger.apply($(process),args);$.fn.trigger.apply($(obj),args);}
listProc.fireWith(process,args);list.fireWith(obj,args);return this;};});process.onCallback=function(event){return function(){process.notify(event,arguments);};};promise.promiseWith(process);promise.promise(source?source:{});if(init){init.call(process,process);}
